...
export const LinkButton = (props: ILinkButtonProps) => {
    ...
    return (
        <a className={`border-0 flex justify-center items-center gap-2 rounded-lg text-base py-2 px-3 h-12 font-medium w-full ${currentStyle.bg} ${currentStyle.text} hover:opacity-80 transition-opacity`} href={props.url}>
            {props.children}
            {props.label}
        </a>
    );
